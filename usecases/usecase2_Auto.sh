...
echo "Booting realms for usecase 2"
echo "Realm A -> shm1 <- Realm B -> shm2 <- Realm C -> shm3 <- Realm A" 
...
qemu-system-aarch64 \
      ...
      -append "console=hvc0 root=/dev/vda1 rw autorun=usecase2_realm_A_Auto.sh" < /dev/hvc1 >/dev/hvc1 &
...
qemu-system-aarch64 \
      ...
      -append "console=hvc0 root=/dev/vda1 rw autorun=usecase2_realm_B_Auto.sh" < /dev/hvc2 >/dev/hvc2 &
...
qemu-system-aarch64 \
      ...
      -cpu host -M virt -enable-kvm -M gic-version=3,its=on \
      -smp 1 -m 1024M -nographic \
      -append "console=hvc0 root=/dev/vda1 rw autorun=usecase2_realm_C_Auto.sh" < /dev/hvc3 >/dev/hvc3 &
