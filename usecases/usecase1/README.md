...
**NOTE** once the realms boot, the ivshmem devices will appear here (numbers 03,04 may vary but they should be exact - you should be able to find them by listing the devices inside the kernel anyway):
```bash
...
Shared memory: **shm1**
Workflow:
1. Putts the .mp4 into the file (use raw.mp4 inside encoding):
```bash
...
1. Reads the file from shared memory, and clears it:
```bash
...
2. Prints something to console, like: "Video uploaded..waiting for next upload"
Then, goes back to step 1.